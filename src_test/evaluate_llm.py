...
def target_check_result(case: dict[str, Any], result: dict[str, Any]) -> dict[str, Any]:
    ...
    candidate_lists = [
        result.get("used_restaurant_list", []) or [],
        result.get("restaurant_list", []) or [],
    ]
...
def build_summary(case_results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for component in ["route", "payload", "target", "answer", "retrieval"]:
        component_accuracy[component] = round(
            sum(1 for item in case_results if item.get("checks", {}).get(component, {}).get("passed")) / total,
            4,
        ) if total else 0.0
...
def detect_environment_failure(case_results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "is_environment_failure": is_env_failure,
        "error_type": first_type if all_same else "",
...
def print_console_report(report: dict[str, Any]) -> None:
    ...
    if env_failure.get("is_environment_failure"):
        ...
        print(
            f"All {env_failure.get('count', 0)} cases failed before evaluation due to "
            f"{env_failure.get('error_type', 'unknown error')}."
        )
    ...
    type_rows = [
        [row["query_type"], row["total"], row["passed"], row["failed"], pct(row["pass_rate"]), f"{row['average_score']:.2f}"]
...
def write_html_report(report: dict[str, Any], output_path: Path) -> None:
    ...
    failed_cases = [item for item in report["cases"] if not item.get("passed")]
    failed_table = html_table(
        ["Status", "Case ID", "Type", "Score", "Route", "Payload", "Target", "Answer", "Retrieval", "Used", "Reason", "Question"],
        build_case_rows(failed_cases),
    ) if failed_cases else "<p class='empty'>All cases passed.</p>"
    ...
    if not preflight.get("ok", True):
        preflight_html = (
            "<section class='panel'>"
            "<h2>Preflight Issues</h2>"
            + "".join(f"<p class='alert'>{html.escape(issue)}</p>" for issue in preflight.get("issues", []))
            + "</section>"
        )
    ...
    if env_failure.get("is_environment_failure"):
        env_failure_html = (
            "<section class='panel'>"
            "<h2>Environment Failure Detected</h2>"
            f"<p class='alert'>All {env_failure.get('count', 0)} cases failed before evaluation due to "
            f"{html.escape(env_failure.get('error_type', 'unknown error'))}.</p>"
            "<p class='alert'>This is not a model-quality result. Fix the runtime environment first, then rerun.</p>"
            "</section>"
        )
...
<body>
  <div class="wrap">
    ...
    <div class="grid-two">
      <section class="panel">
        <h2>Overview</h2>
        {html_table(["Metric", "Value"], overview_rows)}
      </section>
      <section class="panel">
        <h2>By Query Type</h2>
        {html_table(["Type", "Total", "Passed", "Failed", "Pass Rate", "Avg Score"], type_rows)}
      </section>
    </div>
    ...
    <section class="panel">
      <h2>Lowest Score Cases</h2>
      {html_table(["Status", "Case ID", "Type", "Score", "Route", "Payload", "Target", "Answer", "Retrieval", "Used", "Reason", "Question"], build_case_rows(lowest_cases))}
    </section>
  </div>
</body>
</html>
